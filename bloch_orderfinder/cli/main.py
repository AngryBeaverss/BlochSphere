...
def main(argv=None) -> int:
        ...
        if H > 1 and int(eff_stride) != W:
            ...
            if stagger_candidates:
                ...
                for i, c in enumerate(stagger_candidates, 1):
                    support = float(c.detail.get("support_score", 0.0))
                    samples = int(c.detail.get("effective_samples", 0))
                    print(
                        f"  {i:2d}. shift=(dx={c.dx:4d}, dy={c.dy:4d}) "
                        f"score={c.score:.6f} support={support:.3f} n={samples} "
                        f"delta_x={c.delta_x}"
                    )
            else:
    ...
    # Torus candidates retain their L_torus-r complement.  A nonwrapped stagger
    # displacement is not a torus residue, so test its exact positive delta only.
    ...
    for i in range(image_topk):
        ...
        if i < len(stagger_candidates):
            stagger_r = int(stagger_candidates[i].delta_x)
            if stagger_r > 0:
                to_test.append(stagger_r)
    ...
    with open(summary_path, "w", encoding="utf-8") as f:
        ...
        if stagger_candidates:
            ...
            for c in stagger_candidates:
                f.write(
                    f"  dx={c.dx} dy={c.dy} score={c.score} "
                    f"support={c.detail.get('support_score')} "
                    f"samples={c.detail.get('effective_samples')} "
                    f"delta_x={c.delta_x}\n"
                )
        if candidates:
