...
def llg_heun_checkpointed(
    ...
    **kwargs,
    ...
    """Run LLG Heun integration with optional checkpoint callbacks.

    Parameters
    ----------
    steps:
        Total number of *outer* steps (the same 'steps' used by llg_heun).
    save_every:
        If >0, invoke callback every `save_every` steps (and at step 0 if
        include_initial=True).
    callback:
        Callable(step:int, S, diagnostics:Diagnostics|None) -> None
        Called with the current step, the current spin field (on backend), and
        diagnostics (computed on the same backend).
    include_initial:
        Whether to emit step=0 before any evolution.
    diag_J:
        Coupling J used for compute_diagnostics.
    kwargs:
        Forwarded to llg_heun (J, Bext, alpha, gamma, dt, substeps, coherent, ...)
    ...
    Notes
    -----
    This function is intentionally a thin wrapper so it does not change the
    numerical integrator; it only adds a checkpoint cadence.
    """

    total = int(steps)
    every = int(save_every)

    def _emit(step_i, S_i):
        if callback is None:
            return
        diag = compute_diagnostics(backend, S_i, J=float(diag_J))
        callback(int(step_i), S_i, diag)

    if include_initial and (callback is not None):
        _emit(0, S)
    ...
    if every <= 0:
        # No checkpoints: run once.
        return llg_heun(backend, S, steps=total, **kwargs)

    done = 0
    while done < total:
        chunk = min(every, total - done)
        S = llg_heun(backend, S, steps=chunk, **kwargs)
        done += chunk
        _emit(done, S)
