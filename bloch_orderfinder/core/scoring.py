    ...
    *,
    ...
    for dy in range(1, max_dy + 1):
        ...
        for dx in dx_values:
            ...
            effective_samples = int((H - dy) * overlap)
            # Raw normalized correlation has variance that grows as support
            # shrinks.  Use a support-adjusted value only for ranking across
            # different row separations; preserve the raw score for reporting.
            support_score = float(score) * float(np.sqrt(max(1, effective_samples)))
            scores.append((support_score, score, dx, overlap, effective_samples))

        # Retain a small local pool from each row separation before global sort.
        scores.sort(key=lambda item: (-item[0], -item[1], abs(item[2]), item[2]))
        for support_score, score, dx, overlap, effective_samples in scores[: max(topk * 4, topk)]:
            delta_x = int(dx + dy * row_stride)
            if exclude_zero_delta and delta_x == 0:
                continue
            if delta_x in used_delta:
            ...
            candidates.append(
                ShiftScore(
                    ...
                    detail={
                        ...
                        "W": W,
                        "row_stride": row_stride,
                        "row_separation": int(dy),
                        "horizontal_offset": int(dx),
                        "overlap_rows": int(H - dy),
    ...
    candidates.sort(
        key=lambda z: (
            -float(z.detail.get("support_score", 0.0)),
            -z.score,
            abs(z.dy),
            abs(z.dx),
            abs(z.delta_x),
